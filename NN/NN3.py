...
class model(nn.Module):
    ...
    def __init__(self, args,  config):
        ...
        if self.num_layers>1:
            ...
            self.rnn = nn.LSTM(args.features, args.hidden_size, self.num_layers) # rnn
    ...
    def forward(self, batch):
        v = Variable(batch[2]).float().cuda()
        v=v.permute(1, 0, 2)
        vy = Variable(batch[3]).float().cuda()
        vy=vy.permute(1, 0, 2)
